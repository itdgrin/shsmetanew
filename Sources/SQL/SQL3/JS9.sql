SELECT *
FROM
/*Ï‡ÚÂË‡Î˚ ‚ ‡ÒˆÂÌÍ‡ı*/ 
((SELECT
	ssd.sm_id  			as sm_id1,
	ssd1.sm_id 			as sm_id2,
	ssd2.sm_id 			as sm_id3,

	ssd.name  			as c1,
	ssd1.name 			as c2,
	ssd2.name 			as c3,
	ssd.sm_number 			as c4,
	ssd.sm_number 			as c5,
	ssd2.sm_number 			as c6,

	mtc.MAT_CODE			as c7,   
	UCASE(mtc.MAT_NAME) 		as c8,
	mtc.MAT_UNIT			as c9,
	round(sum(mtc.MAT_COUNT),6) 	as c10,

	coast_no_nds			as c11,
	sum(mat_sum_no_nds)		as c12,
	proc_transp			as c13,
	sum(mat_transp_no_nds)		as c14,
	
	'»“Œ√Œ Ã¿“≈–»¿À€'		as c15,
	'“–¿ÕCœŒ–“ Ã¿“≈–»¿ÀŒ¬'		as c16,

	2				as odr1
	

FROM smetasourcedata ssd
INNER JOIN smetasourcedata ssd1 ON ssd1.PARENT_ID = ssd.SM_ID  		AND ssd1.SM_TYPE = 1
INNER JOIN smetasourcedata ssd2 ON ssd2.PARENT_ID = ssd1.SM_ID 		AND ssd2.SM_TYPE = 3
INNER JOIN data_estimate de 	ON de.ID_ESTIMATE = ssd2.SM_ID     	AND de.ID_TYPE_DATA IN (1, 2, 5) 
INNER JOIN card_rate cr 	ON cr.ID = de.ID_TABLES
INNER JOIN materialcard mtc 	ON mtc.ID_CARD_RATE = cr.ID
WHERE ssd.SM_ID =:SM_ID     
group by sm_id2,MAT_CODE,coast_no_nds,proc_transp) 
/******************************************************************************************/
UNION
/******************************************************************************************/
/*Ï‡ÚÂË‡Î˚ ‚ÌÂ ‡ÒˆÂÌÓÍ*/ 
(SELECT
	ssd.sm_id  			as sm_id1,
	ssd1.sm_id 			as sm_id2,
	ssd2.sm_id 			as sm_id3,

	ssd.name  			as c1,
	ssd1.name 			as c2,
	ssd2.name 			as c3,
	ssd.sm_number 			as c4,
	ssd.sm_number 			as c5,
	ssd2.sm_number 			as c6,

	mtc.MAT_CODE			as c7,   
	UCASE(mtc.MAT_NAME) 		as c8,
	mtc.MAT_UNIT			as c9,
	round(sum(mtc.MAT_COUNT),6) 	as c10,

	coast_no_nds			as c11,
	sum(mat_sum_no_nds)		as c12,
	proc_transp			as c13,
	sum(mat_transp_no_nds)		as c14,

	'»“Œ√Œ Ã¿“≈–»¿À€'		as c15,
	'“–¿ÕCœŒ–“ Ã¿“≈–»¿ÀŒ¬'		as c16,

	2				as odr1

FROM smetasourcedata ssd
INNER JOIN smetasourcedata ssd1 ON ssd1.PARENT_ID = ssd.SM_ID  		AND ssd1.SM_TYPE = 1
INNER JOIN smetasourcedata ssd2 ON ssd2.PARENT_ID = ssd1.SM_ID 		AND ssd2.SM_TYPE = 3
INNER JOIN data_estimate de 	ON de.ID_ESTIMATE = ssd2.SM_ID     	AND de.ID_TYPE_DATA IN (1, 2, 5) 
INNER JOIN materialcard mtc 	ON mtc.ID = de.ID_TABLES    		AND mtc.ID_CARD_RATE = 0
WHERE ssd.SM_ID =:SM_ID     
group by sm_id2,MAT_CODE,coast_no_nds,proc_transp)
/******************************************************************************************/
UNION
/******************************************************************************************/
/*ÏÂı‡ÌËÁÏ˚ ‚ ‡ÒˆÂÌÍ‡ı*/ 
(SELECT 
	ssd.sm_id  				as sm_id1,
	ssd1.sm_id 				as sm_id2,
	ssd2.sm_id 				as sm_id3,

	ssd.name  				as c1,
	ssd1.name 				as c2,
	ssd2.name 				as c3,
	ssd.sm_number 				as c4,
	ssd.sm_number 				as c5,
	ssd2.sm_number 				as c6,

	mch.MECH_CODE 				as c7, 
	UCASE(mch.MECH_NAME) 			as c8, 
	mch.MECH_UNIT				as c9,   
	round(sum(mch.MECH_COUNT),6)		as c10,
	
	coast_no_nds				as c11,
	sum(mech_sum_no_nds)			as c12,
	zp_mach_no_nds				as c13,
	sum(zpprice_no_nds)			as c14,

	'»“Œ√Œ Ã¿ÿ»Õ€ » Ã≈’¿Õ»«Ã€'		as c15,
 	'«¿–œÀ¿“¿ Ã¿ÿ»Õ»—“Œ¬'			as c16,

	1					as odr1

 FROM smetasourcedata ssd
 INNER JOIN smetasourcedata ssd1 	ON ssd1.PARENT_ID = ssd.SM_ID  AND ssd1.SM_TYPE = 1 
 INNER JOIN smetasourcedata ssd2	ON ssd2.PARENT_ID = ssd1.SM_ID AND ssd2.SM_TYPE = 3  
 INNER JOIN data_estimate de 		ON de.ID_ESTIMATE = ssd2.SM_ID  
 INNER JOIN card_rate cr 		ON cr.ID = de.ID_TABLES 
 INNER JOIN mechanizmcard mch 		ON mch.ID_CARD_RATE = cr.ID 
 WHERE ssd.SM_ID = :SM_ID   
 group by sm_id2,MECH_CODE,coast_no_nds,zp_mach_no_nds)


/******************************************************************************************/
UNION
/******************************************************************************************/
/*ÏÂı‡ÌËÁÏ˚ ‚ÌÂ ‡ÒˆÂÌÓÍ*/ 
(SELECT 
	ssd.sm_id  				as sm_id1,
	ssd1.sm_id 				as sm_id2,
	ssd2.sm_id 				as sm_id3,

	ssd.name  				as c1,
	ssd.name 				as c2,
	ssd2.name 				as c3,
	ssd.sm_number 				as c4,
	ssd.sm_number 				as c5,
	ssd2.sm_number 				as c6,

	mch.MECH_CODE 				as c7, 
	UCASE(mch.MECH_NAME) 			as c8, 
	mch.MECH_UNIT				as c9,   
	round(sum(mch.MECH_COUNT),6)		as c10,
	
	coast_no_nds				as c11,
	sum(mech_sum_no_nds)			as c12,
	zp_mach_no_nds				as c13,
	sum(zpprice_no_nds)			as c14,

	'»“Œ√Œ Ã¿ÿ»Õ€ » Ã≈’¿Õ»«Ã€'		as c15,
 	'«¿–œÀ¿“¿ Ã¿ÿ»Õ»—“Œ¬'			as c16,

	1					as odr1

 FROM smetasourcedata ssd
 INNER JOIN smetasourcedata ssd1 	ON ssd1.PARENT_ID = ssd.SM_ID  	 AND ssd1.SM_TYPE = 1 
 INNER JOIN smetasourcedata ssd2	ON ssd2.PARENT_ID = ssd1.SM_ID   AND ssd2.SM_TYPE = 3  
 INNER JOIN data_estimate de 		ON de.ID_ESTIMATE = ssd2.SM_ID   and de.ID_TYPE_DATA 
INNER JOIN mechanizmcard mch 		ON mch.ID = de.ID_TABLES 	 AND mch.ID_CARD_RATE = 0
 WHERE ssd.SM_ID = :SM_ID   
 group by sm_id2,MECH_CODE,coast_no_nds,zp_mach_no_nds)


/******************************************************************************************/
UNION
/******************************************************************************************/
/*Á‡Ú‡Ú˚ ÚÛ‰‡*/ 
(select 
	ssd.sm_id  				as sm_id1,
	ssd1.sm_id 				as sm_id2,
	ssd2.sm_id 				as sm_id3,

	ssd.name  				as c1,
	ssd1.name 				as c2,
	ssd2.name 				as c3,
	ssd.sm_number 				as c4,
	ssd.sm_number 				as c5,
	ssd2.sm_number 				as c6,

	"1-1" 					as c7,  
	"«¿“–¿“€ “–”ƒ¿ –¿¡Œ◊»’-—“–Œ»“≈À≈…" 	as c8, 

 
	"˜ÂÎ/˜" 				as c9,  
	round(sum(de.trud),6)			As c10,
	0 					as c11,
	0 					as c12,
	0 					as c13,
	0 					as c14,

	''					as c15,
	''					as c16,

	0					as odr1

FROM smetasourcedata ssd
INNER JOIN smetasourcedata ssd1 ON ssd1.PARENT_ID = ssd.SM_ID  		AND ssd1.SM_TYPE = 1
INNER JOIN smetasourcedata ssd2 ON ssd2.PARENT_ID = ssd1.SM_ID 		AND ssd2.SM_TYPE = 3
INNER JOIN data_estimate de 	ON de.ID_ESTIMATE = ssd2.SM_ID     	
INNER JOIN card_rate cr 	ON cr.ID = de.ID_TABLES
WHERE ssd.SM_ID =:SM_ID     
group by ssd1.sm_id )
/******************************************************************************************/
UNION
/******************************************************************************************/
/*Á‡Ú‡Ú˚ ÚÛ‰‡*/ 
(select 
	ssd.sm_id  				as sm_id1,
	ssd1.sm_id 				as sm_id2,
	ssd2.sm_id 				as sm_id3,

	ssd.name  				as c1,
	ssd1.name 				as c2,
	ssd2.name 				as c3,
	ssd.sm_number 				as c4,
	ssd.sm_number 				as c5,
	ssd2.sm_number 				as c6,

	"1-2" 					as c7,  
	"«¿–¿¡Œ“Õ¿ﬂ œÀ¿“¿ –¿¡Œ◊»’-—“–Œ»“≈À≈…" 	as c8, 

 
	"Û·" 					as c9,  
	round(sum(de.zp),6)			as c10,
	0 					as c11,
	0 					as c12,
	0 					as c13,
	0 					as c14,

	''					as c15,
	''					as c16,

	0					as odr1


FROM smetasourcedata ssd
INNER JOIN smetasourcedata ssd1 ON ssd1.PARENT_ID = ssd.SM_ID  		AND ssd1.SM_TYPE = 1
INNER JOIN smetasourcedata ssd2 ON ssd2.PARENT_ID = ssd1.SM_ID 		AND ssd2.SM_TYPE = 3
INNER JOIN data_estimate de 	ON de.ID_ESTIMATE = ssd2.SM_ID     	
INNER JOIN card_rate cr 	ON cr.ID = de.ID_TABLES
WHERE ssd.SM_ID =:SM_ID     
group by ssd1.sm_id )
/******************************************************************************************/
UNION
/******************************************************************************************/
/*Á‡Ú‡Ú˚ ÚÛ‰‡*/ 
(select 
	ssd.sm_id  				as sm_id1,
	ssd1.sm_id 				as sm_id2,
	ssd2.sm_id 				as sm_id3,

	 
	ssd.name  				as c1,
	ssd1.name 				as c2,
	ssd2.name 				as c3,
	ssd.sm_number 				as c4,
	ssd.sm_number 				as c5,
	ssd2.sm_number 				as c6,

	"1-3" 					as c7,  
	"«¿“–¿“€ “–”ƒ¿ Ã¿ÿ»Õ»—“Œ¬" 		as c8, 

	
	"˜ÂÎ/˜" 				as c9,  
	round(sum(de.trud_mash),6)		As c10,
	0 					as c11,
	0 					as c12,
	0 					as c13,
	0 					as c14,

	''					as c15,
	''					as c16,

	0					as odr1	


FROM smetasourcedata ssd
INNER JOIN smetasourcedata ssd1 ON ssd1.PARENT_ID = ssd.SM_ID  		AND ssd1.SM_TYPE = 1
INNER JOIN smetasourcedata ssd2 ON ssd2.PARENT_ID = ssd1.SM_ID 		AND ssd2.SM_TYPE = 3
INNER JOIN data_estimate de 	ON de.ID_ESTIMATE = ssd2.SM_ID     	
INNER JOIN card_rate cr 	ON cr.ID = de.ID_TABLES
WHERE ssd.SM_ID =:SM_ID     
group by ssd1.sm_id )
/******************************************************************************************/
UNION
/******************************************************************************************/
/*Ú‡ÌÒÔÓÚ*/ 
(SELECT
	ssd.sm_id  			as sm_id1,
	ssd1.sm_id 			as sm_id2,
	ssd2.sm_id 			as sm_id3,

	ssd.name  			as c1,
	ssd1.name 			as c2,
	ssd2.name 			as c3,
	ssd.sm_number 			as c4,
	ssd1.sm_number 			as c5,
	ssd2.sm_number 			as c6,

	mtc.transp_code_just		as c7,
	UCASE(mtc.transp_just)		as c8,
	
	mtc.CARG_UNIT			as c9,
	mtc.transp_COUNT		as c10,


	0				as c11,   
	0				as c12,
	0				as c13,
	0 				as c14,

	'»“Œ√Œ “–¿ÕCœŒ–“'		as c15,
 	'“–¿ÕCœŒ–“ '			as c16,

	3				as odr1

FROM smetasourcedata ssd
INNER JOIN smetasourcedata ssd1 ON ssd1.PARENT_ID = ssd.SM_ID  		AND ssd1.SM_TYPE = 1
INNER JOIN smetasourcedata ssd2 ON ssd2.PARENT_ID = ssd1.SM_ID 		AND ssd2.SM_TYPE = 3
INNER JOIN data_estimate de 	ON de.ID_ESTIMATE = ssd2.SM_ID     	AND de.ID_TYPE_DATA IN (6, 8, 7, 9) 
INNER JOIN transpcard mtc 	ON mtc.ID = de.ID_TABLES 	
WHERE ssd.SM_ID =:SM_ID     
group by transp_code_just,ssd1.sm_id)

/******************************************************************************************/
UNION
/******************************************************************************************/
/*Ò‚‡ÎÍ‡*/ 
(SELECT
	ssd.sm_id  				as sm_id1,
	ssd1.sm_id 				as sm_id2,
	ssd2.sm_id 				as sm_id3,

	ssd.name  				as c1,
	ssd1.name 				as c2,
	ssd2.name 				as c3,
	ssd.sm_number 				as c4,
	ssd1.sm_number 				as c5,
	ssd2.sm_number 				as c6,

	dc.dump_code_just			as c7,
	UCASE(dc.dump_just)			as c8,
	dc.DUMP_UNIT				as c9,
	dc.DUMP_COUNT				as c10,
	


	0					as c11,   
	0 					as c12,
	0					as c13,
	0 					as c14,

	'»“Œ√Œ —¬¿À ¿'				as c15,
 	'“–¿ÕCœŒ–“ Ã”—Œ–¿'			as c16,

	4					as odr1

FROM smetasourcedata ssd
INNER JOIN smetasourcedata ssd1 ON ssd1.PARENT_ID = ssd.SM_ID  		AND ssd1.SM_TYPE 	= 1
INNER JOIN smetasourcedata ssd2 ON ssd2.PARENT_ID = ssd1.SM_ID 		AND ssd2.SM_TYPE 	= 3
INNER JOIN data_estimate de 	ON de.ID_ESTIMATE = ssd2.SM_ID     	AND de.ID_TYPE_DATA 	= 5 
INNER JOIN dumpcard dc 		ON dc.ID =de.ID_TABLES		 		 
WHERE ssd.SM_ID =:SM_ID     
group by dump_code_just,ssd1.sm_id)

/******************************************************************************************/
)    CR_REZ ORDER BY sm_id2,odr1,c7,c15


/*sm_id2,sm_id3,c15,c5,c6,c7 */
